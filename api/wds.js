export default async function handler(req,res){
  ...
  const q="SELECT TOP 30 wds,component_id,spect_type,notes,ra,dec FROM wds WHERE 1=CONTAINS(POINT('ICRS',ra,dec),CIRCLE('ICRS',"+ra+","+dec+",0.0083333333))";
  ...
  try{
    ...
    const idx=n=>meta.findIndex(x=>String(x).toLowerCase()===n), wi=idx("wds"),ci=idx("component_id"),si=idx("spect_type"),ni=idx("notes");
    ...
    const best=Object.values(groups).sort((a,b)=>b.length-a.length)[0]; const notes=best.map(r=>String(r[ni]||"")).join(""); const physical=notes.includes("Z")?true:notes.includes("Y")?false:null;
    ...
    return res.status(200).json({found:true,physical,members:Math.max(2,labels.size),spectral:[...new Set(spectral)].slice(0,6),wds:best[0][wi]});
